...
| utTypeIcons | Modern document icon; see [utTypeIcons](#uttypeicons-macos) below. | macOS    |
...
The **conformsTo** property specifies the Uniform Type Identifier (UTI) that your custom file type conforms to. This tells the system which standard type your format is based on (e.g. plain text, image, or data). Common values include `public.plain-text`, `public.image`, `public.data`, and `public.composite-content`. It maps to `UTTypeConformsTo` in the app’s Info.plist and is used in both exported and imported type declarations.
...
When **utTypeIcons** is set, macOS 11+ uses this modern icon only; **iconName** is **not** used on those systems. The **iconName** icon is still written to the bundle and is used only as a fallback on older macOS versions (pre–11). If you omit **utTypeIcons**, **iconName** is used for the document type on all supported macOS versions.
